...
3. Sinh ra một đoạn văn bản ngắn (1-3 câu) 'overview' phân tích hoặc tóm tắt ý nghĩa của biểu đồ.
...
- ĐỐI VỚI BIỂU ĐỒ ĐƯỜNG (LINE CHART): Phải format rõ trục X nếu trục X là năm (hiển thị số nguyên, ví dụ 2023 thay vì 2,023.2) bằng cách dùng `fig.update_xaxes(tickformat='d', dtick=1)`. Bắt buộc hiển thị marker (dấu chấm) trên line (ví dụ `markers=True` trong `px.line`).
- MÀU SẮC: Đối với tất cả các biểu đồ, BẮT BUỘC sử dụng palette màu 'Paired' (ví dụ: `color_discrete_sequence=px.colors.colorbrewer.Paired`).
- Title, nhãn trục X, Y và Legend phải rõ ràng, ưu tiên tiếng Việt.
- BẮT BUỘC phải dùng template JSON: Bạn trả về chính xác chuỗi JSON có format sau:
```json
{
  "code": "import plotly.express as px\\nfig = px.bar(df, ...)\\nfig.update_layout(...)",
  "overview": "Đoạn văn phân tích ngắn..."
}
```
        ...
        try:
            ...
            try:
                ...
                if fig is not None and save_path is not None:
                    ...
                    # We can use Kaleido or plotly's built-in html export.
                    # Since Kaleido might not be installed, writing to HTML is safest.
                    html_path = save_path.with_suffix(".html")
                    fig.write_html(str(html_path))
                    overview += f"\n\n[System] Đã lưu biểu đồ tại: {html_path}"
