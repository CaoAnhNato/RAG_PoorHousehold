...
6. Phân biệt Huyện và Xã: Huyện/Thành phố lọc theo `"administrative.district"`, Xã/Phường/Thị trấn lọc theo `"administrative.commune"`.
...
8. TẤT CẢ các cột trong GROUP BY BẮT BUỘC phải xuất hiện trong SELECT.""",
            ...
            "join": """9. TRUY VẤN THÀNH VIÊN (BẮT BUỘC JOIN): Khi hỏi về ĐẶC ĐIỂM CỦA TỪNG THÀNH VIÊN (dân tộc thành viên m."member.ethnicity", năm sinh thành viên m."member.birthYear", độ tuổi thành viên), BẮT BUỘC JOIN bảng members m với households h qua: m."administrative.district" = h."administrative.district" AND m."administrative.commune" = h."administrative.commune" AND m."family.hostName" = h."family.hostName" AND m."administrative.year" = h."administrative.year". Khi JOIN LUÔN ghi rõ tiền tố bảng h. hoặc m. cho tất cả các cột (ví dụ h."administrative.year"). Khi đếm số thành viên dùng COUNT(m.*) hoặc SUM(CASE...). TUYỆT ĐỐI KHÔNG lấy cột của chủ hộ (như family.hostBirthYear hay family.ethnicity) khi hỏi về thành viên.""",
    ...
    def generate(self, user_question: str, schema_info: dict) -> str:
        """Sinh câu lệnh SQL dựa trên schema thu gọn và Ultra-Compressed Rules."""
...
{examples_str}"""
        print(f"\n[SQLGenerator] Dynamic Rules Injected: {len(active_rules)} rule groups. Prompt Length: {len(system_prompt)} chars.")
