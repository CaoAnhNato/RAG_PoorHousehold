...
def validate():
    print("1. Đang sinh báo cáo số 11 năm 2024...")
    res = ReportGenerator.generate(report_id=11, year=2024)
    docx_path = Path("Runtime/reports/bao_cao_11_2024.docx")
    ...
    chart_files = [
        "report_11_chart_1.html",
        "report_11_chart_2.html",
        "report_11_chart_3.html"
    ]
    ...
    df, sql, title = execute_report_query(11, 2024)
    dist_df = df[df["STT"].astype(str).str.isdigit()]

    tong_tre_ngheo = int(dist_df["1. Tổng số trẻ em hộ nghèo"].sum()) if "1. Tổng số trẻ em hộ nghèo" in dist_df.columns else 0
    tong_tre_can_ngheo = int(dist_df["4. Tổng số trẻ em hộ cận nghèo"].sum()) if "4. Tổng số trẻ em hộ cận nghèo" in dist_df.columns else 0

    print(f"   -> Tổng số trẻ em hộ nghèo toàn tỉnh từ DuckDB: {tong_tre_ngheo:,}".replace(",", "."))
    print(f"   -> Tổng số trẻ em hộ cận nghèo toàn tỉnh từ DuckDB: {tong_tre_can_ngheo:,}".replace(",", "."))
    ...
    for _, r in dist_df.iterrows():
        ...
        yt_ngheo = int(r.get("2. Y tế hộ nghèo", 0))
        gd_ngheo = int(r.get("3. Giáo dục hộ nghèo", 0))
        if ten_dv in clean_text and str(yt_ngheo) in clean_text and str(gd_ngheo) in clean_text:
            print(f"   [THÀNH CÔNG] Đơn vị '{ten_dv}': Khớp chính xác Y tế Nghèo={yt_ngheo}, Giáo dục Nghèo={gd_ngheo}")
        else:
            print(f"   [CẢNH BÁO] Sai lệch số liệu tại đơn vị '{ten_dv}': Y tế={yt_ngheo}, Giáo dục={gd_ngheo}")
            parity_ok = False
    ...
    required_analysis_phrases = [
        "Biểu đồ cột nhóm minh họa rõ nét 'sự bất bình đẳng khởi điểm'",
        "Bản đồ bong bóng khi soi chiếu ở cấp Xã cho thấy nhiều đơn vị hành chính",
        "Biểu đồ phân tán tương quan vốn người ở trẻ em Cận nghèo phản ánh một nghịch lý"
    ]
    missing_analysis = []
    ...
    print("\n=> TẤT CẢ KIỂM ĐỊNH BÁO CÁO SỐ 11 ĐẠT 100% CHUẨN XÁC!\n")
    return True
